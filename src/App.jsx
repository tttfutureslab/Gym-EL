...
// ─── CARDIO ──────────────────────────────────────────────────────────────────
const CARDIO_M = {
  elliptical: { min: 1.8, tip: "Nivel 6-8, cadencia 130-150ppm. Sala cardio Aqua — siempre al final del entreno." },
  treadmill:  { min: 1.5, tip: "Trote 9km/h o inclinación 8% a 6km/h. 10 min post-entreno para oxidar ácido láctico." },
};

// ─── YOUTUBE VIDEO IDs per exercise ──────────────────────────────────────────
// Official Technogym / Hammer Strength / reputable fitness tutorials
const VIDEOS = {
  // LUNES - PUSH
  "HS Chest Press": "3a9SC2DFb4o",
  "HS Incline Press": "IP1pNJPORww",
  "TG Pec Deck": "x9kPD0hJ0bQ",
  "HS Shoulder Press": "qEwKCR5JCog",
  "TG Lateral Raise": "3VcKaXpzqRo",
  "TG Tricep Pushdown": "2-LAMcpzODU",
  "HS Dip": "l8bJjmkFHPs",
  // MARTES - PULL
  "HS Wide Row": "GZbfZ033f74",
  "TG Lat Pulldown": "CAwf7n6Tuhs",
  "HS One Arm Row": "pYcpY20QaE8",
  "TG Seated Row": "GZbfZ033f74",
  "TG Face Pull": "0Po47vvj9g4",
  "TG Preacher Curl": "fIWP-FRFNU0",
  "TG Cable Curl": "av7-8CzC9_Y",
  // MIÉRCOLES - LEGS
  "TG Leg Press": "IZxyjW7WGPM",
  "TG Leg Extension": "YyvSfVjQeL0",
  "HS Hack Squat": "EdtpSCFDrBA",
  "TG Lying Leg Curl": "1Tq3QdYUuHs",
  "TG Seated Leg Curl": "1Tq3QdYUuHs",
  "TG Hip Thrust": "SEdqd1n0cvg",
  "TG Calf Raise": "gwLzBJYoWlA",
  // JUEVES - PUSH 2
  "HS Unilateral Shoulder": "qEwKCR5JCog",
  "TG Cable Lateral": "3VcKaXpzqRo",
  "TG Reverse Fly": "1dSfBSGBpWQ",
  "TG Front Raise": "gkaBMfcP-oA",
  "HS Incline Chest": "IP1pNJPORww",
  "TG Overhead Tricep": "KCCL2o6thkI",
  "TG Tricep Machine": "2-LAMcpzODU",
  // VIERNES - PULL 2
  "HS Pulldown": "CAwf7n6Tuhs",
  "TG Neutral Pulldown": "CAwf7n6Tuhs",
  "HS Single Row": "pYcpY20QaE8",
  "TG Pullover": "1rdYNDpCVKo",
  "TG Cable Hammer": "av7-8CzC9_Y",
  "TG Ab Crunch": "Xyd_fa5zoEU",
};

// ─── ROUTINE ─────────────────────────────────────────────────────────────────
const ROUTINE = {
  Lunes: {
    type:"PUSH", muscle:"Pecho · Hombros · Tríceps", emoji:"💪", color:"#f97316",
    tip:"Empieza en la sala Hammer Strength con el press de pecho. Ajusta el asiento antes de cargar peso.",
    exercises:[
      {id:1,name:"HS Chest Press",      display:"HS Press de Pecho",              sets:"5 × 6-8",   muscle:"Pecho",           video:"HS Chest Press",         note:"Sala HS — asiento a altura de pecho, codos a 90°"},
      {id:2,name:"HS Incline Press",    display:"HS Press Inclinado",             sets:"4 × 8-10",  muscle:"Pecho alto",      video:"HS Incline Press",       note:"Sala HS — foco en clavicular"},
      {id:3,name:"TG Pec Deck",         display:"TG Pec Deck (Aperturas)",        sets:"3 × 12-15", muscle:"Pecho",           video:"TG Pec Deck",            note:"Sala LIFT Technogym — stretch completo en cada rep"},
      {id:4,name:"HS Shoulder Press",   display:"HS Press de Hombros",            sets:"4 × 8-10",  muscle:"Hombros",         video:"HS Shoulder Press",      note:"Sala HS — espalda pegada al respaldo"},
      {id:5,name:"TG Lateral Raise",    display:"TG Elevaciones Laterales",       sets:"4 × 15",    muscle:"Hombros",         video:"TG Lateral Raise",       note:"Sala LIFT — codos ligeramente flexionados"},
      {id:6,name:"TG Tricep Pushdown",  display:"TG Extensiones Tríceps Cable",   sets:"3 × 15",    muscle:"Tríceps",         video:"TG Tricep Pushdown",     note:"Cable Station LIFT — codos fijos al cuerpo"},
      {id:7,name:"HS Dip",              display:"HS Fondos en Máquina",           sets:"3 × 12",    muscle:"Tríceps/Pecho",   video:"HS Dip",                 note:"Sala HS — rango completo controlado"},
    ],
  },
  Martes: {
    type:"PULL", muscle:"Espalda · Bíceps · Romboides", emoji:"🏋️", color:"#3b82f6",
    tip:"Foco en llevar los codos atrás. En el remo HS empuja el pecho hacia la máquina para maximizar el rango.",
    exercises:[
      {id:1,name:"HS Wide Row",         display:"HS Remo Agarre Ancho",           sets:"5 × 6-8",   muscle:"Espalda total",   video:"HS Wide Row",            note:"Sala HS — el mejor ejercicio de espalda del gym"},
      {id:2,name:"TG Lat Pulldown",     display:"TG Jalón al Pecho",              sets:"4 × 10",    muscle:"Dorsal",          video:"TG Lat Pulldown",        note:"Sala LIFT — agarre ancho, codos abajo y atrás"},
      {id:3,name:"HS One Arm Row",      display:"HS Remo Unilateral",             sets:"4 × 8 c/l", muscle:"Espalda media",   video:"HS One Arm Row",         note:"Sala HS — rango máximo, codo al techo"},
      {id:4,name:"TG Seated Row",       display:"TG Remo Sentado",                sets:"3 × 12",    muscle:"Romboides",       video:"TG Seated Row",          note:"Cable Station LIFT — pausa 1s contrayendo"},
      {id:5,name:"TG Face Pull",        display:"TG Face Pulls Polea",            sets:"3 × 20",    muscle:"Deltoides post.", video:"TG Face Pull",           note:"Cable Station LIFT — salud del manguito rotador"},
      {id:6,name:"TG Preacher Curl",    display:"TG Curl Bíceps Máquina",         sets:"4 × 10",    muscle:"Bíceps",          video:"TG Preacher Curl",       note:"Sala LIFT — aislamiento total del bíceps"},
      {id:7,name:"TG Cable Curl",       display:"TG Curl en Cable",               sets:"3 × 12",    muscle:"Bíceps/Braquial", video:"TG Cable Curl",          note:"Cable Station — tensión en posición estirada"},
    ],
  },
  Miércoles: {
    type:"LEGS", muscle:"Cuádriceps · Isquios · Glúteos", emoji:"🦵", color:"#10b981",
    tip:"Día más duro — donde más músculo ganarás. Hidratación doble. En el leg press Technogym puedes cargar fuerte con total seguridad.",
    exercises:[
      {id:1,name:"TG Leg Press",        display:"TG Prensa de Piernas 45°",       sets:"5 × 8-10",  muscle:"Cuád/Glúteos",    video:"TG Leg Press",           note:"Sala LIFT — pies altos y anchos para más glúteo"},
      {id:2,name:"TG Leg Extension",    display:"TG Extensiones Cuádriceps",      sets:"4 × 12-15", muscle:"Cuádriceps",      video:"TG Leg Extension",       note:"Sala LIFT — pausa 1s arriba, baja controlado"},
      {id:3,name:"HS Hack Squat",       display:"HS Hack Squat Máquina",          sets:"4 × 10",    muscle:"Cuádriceps",      video:"HS Hack Squat",          note:"Sala HS — rodillas hacia fuera, profundidad completa"},
      {id:4,name:"TG Lying Leg Curl",   display:"TG Curl Femoral Tumbado",        sets:"4 × 10-12", muscle:"Isquios",         video:"TG Lying Leg Curl",      note:"Sala LIFT — rango completo, sin tirones"},
      {id:5,name:"TG Seated Leg Curl",  display:"TG Curl Femoral Sentado",        sets:"3 × 12",    muscle:"Isquios",         video:"TG Seated Leg Curl",     note:"Sala LIFT — complementa al tumbado"},
      {id:6,name:"TG Hip Thrust",       display:"TG Hip Thrust / Glute Press",    sets:"4 × 12",    muscle:"Glúteos",         video:"TG Hip Thrust",          note:"Sala LIFT — pausa 1s arriba, clave para glúteo"},
      {id:7,name:"TG Calf Raise",       display:"TG Gemelos en Máquina",          sets:"5 × 20",    muscle:"Gemelos",         video:"TG Calf Raise",          note:"Sala LIFT — paso completo, stretch abajo"},
    ],
  },
  Jueves: {
    type:"PUSH", muscle:"Hombros · Pecho inclinado · Tríceps", emoji:"⚡", color:"#f97316",
    tip:"Segundo push — más volumen en hombros. La HS unilateral es perfecta para corregir asimetrías entre lados.",
    exercises:[
      {id:1,name:"HS Unilateral Shoulder",display:"HS Press Hombros Unilateral",  sets:"4 × 10 c/l",muscle:"Hombros",         video:"HS Unilateral Shoulder", note:"Sala HS — trabaja cada lado por separado"},
      {id:2,name:"TG Cable Lateral",    display:"TG Elevaciones Laterales Cable", sets:"4 × 15",    muscle:"Hombros",         video:"TG Cable Lateral",       note:"Cable Station LIFT — tensión constante todo el rango"},
      {id:3,name:"TG Reverse Fly",      display:"TG Pájaro / Deltoides Posterior",sets:"3 × 15",    muscle:"Deltoides post.", video:"TG Reverse Fly",         note:"Sala LIFT — codos ligeramente flexionados"},
      {id:4,name:"TG Front Raise",      display:"TG Elevaciones Frontales Cable", sets:"3 × 12",    muscle:"Hombros front.",  video:"TG Front Raise",         note:"Cable Station — barra corta o cuerda"},
      {id:5,name:"HS Incline Chest",    display:"HS Press Inclinado (2º día)",    sets:"4 × 10",    muscle:"Pecho alto",      video:"HS Incline Press",       note:"Sala HS — complementa al lunes"},
      {id:6,name:"TG Overhead Tricep",  display:"TG Extensión Tríceps Cabeza",    sets:"3 × 15",    muscle:"Tríceps",         video:"TG Overhead Tricep",     note:"Cable Station — cuerda, cabeza larga del tríceps"},
      {id:7,name:"TG Tricep Machine",   display:"TG Press Tríceps Máquina",       sets:"3 × 12",    muscle:"Tríceps",         video:"TG Tricep Machine",      note:"Sala LIFT — rango completo"},
    ],
  },
  Viernes: {
    type:"PULL", muscle:"Espalda ancho · Bíceps · Core", emoji:"🔥", color:"#3b82f6",
    tip:"Último día — a dejarlo todo. HS pulldown para conseguir la V. Termina con abdominales en la máquina Technogym.",
    exercises:[
      {id:1,name:"HS Pulldown",         display:"HS Pulldown Agarre Ancho",       sets:"4 × 8-10",  muscle:"Dorsal (ancho)",  video:"HS Pulldown",            note:"Sala HS — el mejor para conseguir la espalda en V"},
      {id:2,name:"TG Neutral Pulldown", display:"TG Jalón Agarre Neutro",         sets:"4 × 10",    muscle:"Dorsal",          video:"TG Neutral Pulldown",    note:"Sala LIFT — agarre neutro o supino"},
      {id:3,name:"HS Single Row",       display:"HS Remo a Una Mano",             sets:"3 × 10 c/l",muscle:"Espalda media",   video:"HS Single Row",          note:"Sala HS — rango máximo, codo al techo"},
      {id:4,name:"TG Pullover",         display:"TG Pull-over Máquina",           sets:"3 × 15",    muscle:"Dorsal/Serrato",  video:"TG Pullover",            note:"Sala LIFT — brazos extendidos, trabajo de serrato"},
      {id:5,name:"TG Cable Hammer",     display:"TG Curl Martillo en Cable",      sets:"3 × 12",    muscle:"Bíceps/Braquial", video:"TG Cable Hammer",        note:"Cable Station — cuerda, supinación al subir"},
      {id:6,name:"TG Cable Curl",       display:"TG Curl Bíceps en Cable",        sets:"3 × 15",    muscle:"Bíceps",          video:"TG Cable Curl",          note:"Cable Station — tensión en posición estirada"},
      {id:7,name:"TG Ab Crunch",        display:"TG Crunch Abdominal Máquina",    sets:"4 × 20",    muscle:"Core",            video:"TG Ab Crunch",           note:"Sala LIFT — control total, sin tirar del cuello"},
    ],
  },
};
...
const sessionKey = (day) => "m5-"+day+"-"+new Date().getFullYear()+"-W"+weekNum();
const prevKey    = (day) => { const p=new Date(Date.now()-7*864e5); const s=new Date(p.getFullYear(),0,1); return "m5-"+day+"-"+p.getFullYear()+"-W"+Math.ceil(((p-s)/86400000+s.getDay()+1)/7); };
...
export default function App() {
  ...
  const [chat,      setChat]      = useState([{role:"assistant",text:"¡Hola! 👋 Rutina PPL diseñada para el Aqua de Vilanova — Technogym LIFT + Hammer Strength. Toca cada ejercicio para ver el video y registrar el peso por serie. ¡Vamos!"}]);
  ...
  useEffect(()=>{
    try{
      const s=localStorage.getItem("m5-sessions"); if(s) setSessions(JSON.parse(s));
      const h=localStorage.getItem("m5-history");  if(h) setHistory(JSON.parse(h));
    }catch(e){}
    setStorageOk(true);
  },[]);
  useEffect(()=>{ if(!storageOk) return; try{localStorage.setItem("m5-sessions",JSON.stringify(sessions));}catch(e){} },[sessions,storageOk]);
  useEffect(()=>{ if(!storageOk) return; try{localStorage.setItem("m5-history",JSON.stringify(history));}catch(e){} },[history,storageOk]);
  ...
  const sendMsg = async () => {
    ...
    const cd=cur.cardio||{};
    const sys="Entrenador personal experto, español natural y directo. PERFIL: hombre 36 años 177cm 90kg, objetivo hipertrofia, PPL 5 días. Gym: Aqua Sport Clubs Vilanova — sala LIFT Technogym + sala Hammer Strength. HOY "+selDay+" ("+day.type+"): "+done+"/"+total+" ejs. "+exSum+". Cardio: "+(cd.km?cd.km+"km":"pendiente")+". Responde 2-4 frases, sin markdown.";
    try{
    ...
    return(
      <div key={ex.id} style={{borderRadius:12,border:s.done?"1px solid rgba(74,222,128,0.2)":"1px solid #141414",background:s.done?"rgba(74,222,128,0.04)":"#0f0f0f",overflow:"hidden",transition:"all 0.2s"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"11px 13px",cursor:"pointer"}} onClick={()=>setExpanded(isExp?null:ex.id)}>
          <div style={{fontSize:9,fontWeight:800,padding:"2px 5px",borderRadius:4,background:isTG?"#3b82f620":isHS?"#f9731620":"#333",color:isTG?"#3b82f6":isHS?"#f97316":"#666",flexShrink:0,letterSpacing:0.5}}>{isTG?"TG":"HS"}</div>
        ...
        {isExp&&(
          <div style={{padding:"0 13px 13px",borderTop:"1px solid #141414"}}>
            <div style={{paddingTop:10,display:"flex",flexDirection:"column",gap:10}}>
                  ...
                  return(
                    <div key={i} style={{marginBottom:9}}>
                      ...
                      <div style={{display:"flex",gap:5,alignItems:"center"}}>
                        <button onClick={()=>setSeriesW(ex.id,i,String(Math.max(0,(parseFloat(sw)||0)-2.5)))} style={{width:30,height:30,borderRadius:7,border:"1px solid #222",background:"#141414",color:"#888",fontSize:15,cursor:"pointer"}}>−</button>
                        ...
                        <button onClick={()=>setSeriesW(ex.id,i,String((parseFloat(sw)||0)+2.5))} style={{width:30,height:30,borderRadius:7,border:"1px solid #222",background:"#141414",color:"#888",fontSize:15,cursor:"pointer"}}>+</button>
              ...
              <button onClick={()=>{setEx(ex.id,"done",!s.done);setExpanded(null)}} style={{padding:"9px",borderRadius:10,border:s.done?"1px solid rgba(74,222,128,0.3)":"none",background:s.done?"rgba(74,222,128,0.15)":"linear-gradient(135deg,"+dc+",#ef4444)",color:s.done?"#4ade80":"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
    ...
    <div style={{minHeight:"100vh",background:"#080808",fontFamily:"'DM Sans',sans-serif",color:"#f0f0f0",maxWidth:480,margin:"0 auto",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{padding:"18px 20px 12px",borderBottom:"1px solid #141414",background:"#0a0a0a"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:900,letterSpacing:2,color:"#fff",lineHeight:1}}>PPL MUSCLE PLAN</div>
            <div style={{fontSize:10,color:"#444",marginTop:3,letterSpacing:1.5,textTransform:"uppercase"}}>Aqua Vilanova · TG + HS · 36a · 90kg</div>
          </div>
        ...
        {showProf&&(
          <div style={{marginTop:10,padding:"12px 14px",borderRadius:10,background:"#111",border:"1px solid #1e1e1e",display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {[["Edad","36 años"],["Altura","177 cm"],["Peso","~90 kg"],["Gym","Aqua — LIFT + HS"]].map(([k,v])=>(
        ...
        <div style={{marginTop:10,display:"flex",alignItems:"center",gap:8}}>
          <div style={{flex:1,height:3,background:"#141414",borderRadius:2}}>
            <div style={{height:"100%",width:wp+"%",background:"linear-gradient(90deg,"+dc+",#ef4444)",borderRadius:2,transition:"width 0.4s"}}/>
          </div>
      ...
      <div style={{flex:1,padding:"14px 20px 24px",overflow:"auto"}}>
        ...
        {view==="done"&&(
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:400,gap:16,textAlign:"center"}}>
            ...
            <div style={{display:"flex",gap:8,marginTop:8}}>
              ...
              <button onClick={()=>setView("history")} style={{padding:"10px 20px",borderRadius:12,border:"none",background:"linear-gradient(135deg,"+dc+",#ef4444)",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Ver historial</button>
            </div>
          </div>
        )}
        ...
        {view==="routine"&&(()=>{
          ...
          const cd=cur.cardio||{}; const ct=cd.type||"elliptical"; const ci=CARDIO_M[ct];
          return(
            <div>
              <div style={{marginBottom:12}}>
                ...
                <div style={{display:"flex",gap:8,marginTop:8}}>
                  ...
                  <span style={{fontSize:10,color:"#f97316",background:"#f9731620",padding:"2px 8px",borderRadius:4,fontWeight:700}}>HS = Sala Hammer Strength</span>
                </div>
              </div>
                ...
                <div style={{borderRadius:12,border:"1px solid #1e1e1e",background:"#0c0c0c",padding:"12px 13px",marginTop:4}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <div style={{fontSize:10,color:"#555",marginTop:1}}>{"Mínimo recomendado: "}<span style={{color:dc,fontWeight:600}}>{ci.min+"km"}</span></div>
                    </div>
                  </div>
                  ...
                  {cd.km&&parseFloat(cd.km)<ci.min&&<div style={{marginTop:6,fontSize:10,color:"#f87171",textAlign:"center"}}>{"⚠️ Por debajo del mínimo ("+ci.min+"km)"}</div>}
              ...
              <button onClick={()=>setView("chat")} style={{marginTop:8,width:"100%",padding:"12px",borderRadius:12,border:"1px solid #1e1e1e",background:"transparent",color:"#666",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>💬 Hablar con mi entrenador</button>
            </div>
          );
        })()}
        ...
        {view==="chat"&&(
          <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 270px)"}}>
            <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:10,paddingBottom:8}}>
              {chat.map((msg,i)=>(
                <div key={i} style={{display:"flex",justifyContent:msg.role==="user"?"flex-end":"flex-start",alignItems:"flex-end",gap:7}}>
                  {msg.role==="assistant"&&<div style={{width:26,height:26,borderRadius:"50%",background:dc+"22",border:"1px solid "+dc+"44",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,flexShrink:0}}>🏋️</div>}
                  <div style={{maxWidth:"80%",padding:"9px 13px",borderRadius:msg.role==="user"?"16px 16px 4px 16px":"16px 16px 16px 4px",background:msg.role==="user"?"linear-gradient(135deg,"+dc+",#ef4444)":"#111",border:msg.role==="assistant"?"1px solid #1a1a1a":"none",fontSize:12,lineHeight:1.65,color:"#f0f0f0",whiteSpace:"pre-wrap"}}>{msg.text}</div>
                </div>
              ))}
              {loading&&(<div style={{display:"flex",alignItems:"flex-end",gap:7}}><div style={{width:26,height:26,borderRadius:"50%",background:dc+"22",border:"1px solid "+dc+"44",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12}}>🏋️</div><div style={{display:"flex",gap:4,padding:"9px 13px",background:"#111",borderRadius:"16px 16px 16px 4px",border:"1px solid #1a1a1a"}}>{[0,1,2].map(i=><div key={i} style={{width:5,height:5,borderRadius:"50%",background:dc,animation:"pulse 1.2s "+i*0.2+"s infinite"}}/>)}</div></div>)}
              <div ref={chatEnd}/>
            </div>
            <div style={{display:"flex",gap:7,paddingTop:10,borderTop:"1px solid #141414"}}>
              ...
              <button onClick={sendMsg} disabled={loading} style={{padding:"10px 15px",borderRadius:12,border:"none",background:loading?"#141414":"linear-gradient(135deg,"+dc+",#ef4444)",color:"#fff",fontSize:14,cursor:loading?"default":"pointer"}}>➤</button>
            </div>
          </div>
        )}
